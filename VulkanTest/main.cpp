...
#pragma region VULKAN_init
	...
	void createGraphicsPipeline()
	{
		...
		// Basically, this allows one to only keep fragments that fall within the specified depth range.
		depthStencilInfo.depthBoundsTestEnable = VK_FALSE;
		depthStencilInfo.minDepthBounds = 0.0f; // optional, because we set depthBoundsTestEnable to false
		depthStencilInfo.maxDepthBounds = 1.0f; // optional, because we set depthBoundsTestEnable to false
		depthStencilInfo.stencilTestEnable = VK_FALSE;
		depthStencilInfo.front = { }; // Optional, because we set the previous setting to false
		depthStencilInfo.back = { }; // Optional, because we set the previous setting to false
		...
		pipelineInfo.pDepthStencilState = &depthStencilInfo; // Optional, when render pass does not contain depth attachement
	...
	void createRenderPass()
	{
		...
		// Our array consists of two VkAttachmentDescription and the first one is the color attachement, hence 0.
	...
	void createBuffer(VkDeviceSize size, VkBufferUsageFlags bufUsage, VmaMemoryUsage memUsage, VkBuffer& buffer, VmaAllocation& allocation)
	{
		...
		if (res != VK_SUCCESS)
		{
			throw std::runtime_error("Failed to VMA allocate buffer!");
		}
	}
	...
	void createImage(uint32_t width, uint32_t height, VkFormat format, VkImageTiling tiling, VkImageUsageFlags imgUsage, VmaMemoryUsage memUsage, VkImage& image, VmaAllocation& imageMemory)
	{
		...
		VkResult res = vmaCreateImage(m_allocator, &imageInfo, &allocInfo, &image, &imageMemory, nullptr);
		if (res != VK_SUCCESS)
		{
			throw std::runtime_error("Failed to VMA allocate image!");
		}
	}
	...
	VkFormat findSupportedFormat(const std::vector<VkFormat>& candidates, VkImageTiling tiling, VkFormatFeatureFlags features) const
	{
		...
		// If none of the candidate formats supports the desired usage, then we can throw an exception.
		throw std::runtime_error("Failed to find supported format!");
	}

	VkFormat findDepthFormat() const
	{
		// selects a format with a depth component that supports usage as depth attachment
	...
	// a helper function that tells us if the chosen depth format contains a stencil component
	...
	void createDepthResources()
	{
		...
		// We could do this in the render pass like the color attachment, but here I've chosen to use a pipeline barrier because the transition only needs to happen once.
...
#pragma endregion
	...
	void updateUniformBuffer(const uint32_t currentImage)
	{
		...
		*/
		...
		std::memcpy(data, &ubo, sizeof(ubo));
		vmaUnmapMemory(m_allocator, m_uniformBuffersMemory[currentImage]);
	}
